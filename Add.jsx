...
const Add = () => {
  ...
  const [empCity, setEmpCity] = useState(''); // New state for emp_city
  const [empSalary, setEmpSalary] = useState(''); // New state for emp_salary
  const [empDesignation, setEmpDesignation] = useState(''); // New state for emp_designation
  ...
  const handleSubmit = (e) => {
    ...
    if (!name || !email || !password || !gender || !courses || !date || !status || !empCity || !empSalary || !empDesignation) {
    ...
    let obj = {
      ...
      date,
      status,
      empCity, // Add emp_city to the object
      empSalary, // Add emp_salary to the object
      empDesignation, // Add emp_designation to the object
    };
    ...
    setPassword('');
    setStatus('');
    setEmpCity(''); // Reset emp_city
    setEmpSalary(''); // Reset emp_salary
    setEmpDesignation(''); // Reset emp_designation
    navigate('/view');
  };
        ...
        <div className="row">
          <div className="col-lg-8 mx-auto mt-4 p-4 mb-5" style={{ border: "2px solid transparent", borderRadius: "10px 20px 10px 20px", boxShadow: "5px 5px 2px #777764" }}>
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-3">
                <label htmlFor="empCity" className="form-label">City :  </label>
                <input type="text" className="form-control" id="empCity" onChange={(e) => setEmpCity(e.target.value)} value={empCity} />
              </div>

              <div className="mb-3">
                <label htmlFor="empSalary" className="form-label">Salary :  </label>
                <input type="number" className="form-control" id="empSalary" onChange={(e) => setEmpSalary(e.target.value)} value={empSalary} />
              </div>

              <div className="mb-3">
                <label htmlFor="empDesignation" className="form-label">Designation :  </label>
                <input type="text" className="form-control" id="empDesignation" onChange={(e) => setEmpDesignation(e.target.value)} value={empDesignation} />
              </div>
